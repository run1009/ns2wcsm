...
//we haven't use it in the centrial scheduling, but use class topology instead
struct WimshMshCscf {
  //channels available for centralized schedulings
  std::vector<int> channels;

  //configuration frame sequence
  unsigned frame_num;
  ...
  struct NodeInf {
    ...
    struct ChildInf {
      ...
      wimax::BurstProfile up;
      wimax::BurstProfile down;
      //in bits
      static unsigned size() { return 16; }
    };
    
    std::list<ChildInf*> children;
    //in bits
    unsigned size() {return (children.size() * ChildInf::size() + 24); }
  };
  
  std::list<NodeInf*> nodes;
  //TODO:add size()
...
private:
  ...
  WimshMshCsch* mshCsch_;
  WimshMshCscf* mshCscf_;
	//------------------//
	//-- Other fields --//
	//------------------//
...
public:
  ...
  void addMshCsch(WimshMshCsch* m);

  WimshMshCsch*& mshCsch () { return mshCsch_; }
  //void addMshCscf(WimshMshCscf* m);
